...
function render(): void {
  ...
  ensureStyle(); document.getElementById(PANEL_ID)?.remove();
  const history = readHistory();
  const finalState: ProofStateView = { theoremName: "ui", completed: false, goals: [{ id: "ui", target: readGoal(), context: [] }] };
  const steps = history.map((tactic) => narrateStep(tactic, null, finalState));
  const panel = document.createElement("section"); panel.id = PANEL_ID; panel.className = "proof-narration-widget"; panel.setAttribute("aria-label", "English proof explanation");
...
let scheduled = false;
const observer = new MutationObserver(() => { if (scheduled) return; scheduled = true; queueMicrotask(() => { scheduled = false; observer.disconnect(); render(); const app = document.querySelector("#app"); if (app) observer.observe(app, { childList: true, subtree: true }); }); });
function start(): void { const app = document.querySelector("#app"); if (!app) return; observer.observe(app, { childList: true, subtree: true }); render(); }
