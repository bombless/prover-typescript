...
test("large Eq proof expression keeps every parenthesis group formatted recursively", () => {
  const source = `( x : Nat ) -> ( x : Nat ) -> Eq Nat ( ( ( fun x : Nat => ( fun x : Nat => ( Nat.rec ( fun x : Nat => Nat ) #0 ( fun x : Nat => ( fun x : Nat => ( Succ #0 ) ) ) #1 ) ) ) #1 ) ( Succ #0 ) ) ( Succ ( ( ( fun x : Nat => ( fun x : Nat => ( Nat.rec ( fun x : Nat => Nat ) #0 ( fun x : Nat => ( fun x : Nat => ( Succ #0 ) ) ) #1 ) ) ) #0 ) ) )`;
